...
#         else:
#             state_t = torch.from_numpy(state).permute(0, 3, 1, 2).float().to(self.device)
...
#
#         if self.replayBuffer.length() < self.config.batch_size:
#             return # pass
#
...
#
#             # 버퍼에 기록
#             self.replayBuffer.append_sample(state, action, reward, next_state, done)
#
#             # 상태 정보 업데이트
#             state = next_state
#
...
#                 curr_loss = self.optimize_policy_model(done)
#                 self.summary_writer.add_scalar("TD_Loss", curr_loss.detach().numpy(), self.current_iteration)
#
#                 # 타겟 네트워크 업데이트
...
#
#         # to torch.Tensor
#         state_tensor = torch.from_numpy(states).permute(0, 3, 1, 2).float()
#         action_tensor = torch.from_numpy(actions).long().unsqueeze(1)
#         reward_tensor = torch.from_numpy(rewards).float().unsqueeze(1)
#         next_state_tensor = torch.from_numpy(next_states).permute(0, 3, 1, 2).float()
#         done_tensor = torch.from_numpy(dones).unsqueeze(1).float()