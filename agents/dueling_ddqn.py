...
from graph.models.dqn import DQN, DuelingDQN, DuelingDQN_v2
...
class DuelingDDQNAgent(BaseAgent):
    def __init__(self, config):
        ...
        # model
        self.policy_model = DuelingDQN_v2(self.config)
        self.target_model = DuelingDQN_v2(self.config)
    ...
    def optimize_policy_model(self, done):
        ...
        current_state_value_local = self.policy_model(state_tensor)
        # state-action(t)의 예측 가치 (bsz x 1) - 선택한 action을 이용
        current_action_value_prediction = current_state_value_local.gather(1, action_tensor)

        # Y_t = R_t+1 + gamma * target_Q(next_state, next_best_local_action)
        # state(t+1)의 타겟 가치 (bsz x action_dim)
        with torch.no_grad():
            next_state_value_target = self.target_model(next_state_tensor).detach()
        # 다음 로컬 액션 중 가장 가치가 높은 액션
        next_state_value_local = self.policy_model(next_state_tensor).detach()
        next_best_action_local = next_state_value_local.max(1)[1].unsqueeze(1)
        # 로컬 액션으로 타겟 스테이트 가치를 뽑는다.
        next_action_value_target = next_state_value_target.gather(1, next_best_action_local)

        # expected Q values
        expected_action_value_target = (reward_tensor + (self.config.gamma * next_action_value_target * (1-done_tensor)))

        # loss: HuberLoss(current prediction, estimated target)
        loss_batch = self.loss(current_action_value_prediction, expected_action_value_target)
    ...
    def train_one_epoch(self, ep_num):
        ...
        while not done:
            ...
            if ep_num > self.config.start_train_episode:
                ...
                # 타겟 네트워크 업데이트
                self.soft_update(self.policy_model, self.target_model)
        ...
        if episode_rewards_running_average > self.best_score:
            ...
            self.summary_writer.add_scalar("Training_best_score", self.best_score, self.current_episode)
